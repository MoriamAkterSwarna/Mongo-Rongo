...
ধরা যাক, আমাদের একটি `students` কালেকশন আছে যেখানে প্রতিটি ডকুমেন্টে students দের নাম, ক্লাস এবং রেজাল্টের তথ্য আছে। আমরা এমন students দের খুঁজতে চাই যারা `class: "10"` এবং `result: "A"`।
...
উপরের একই `students` কালেকশনের ক্ষেত্রে, এবার আমরা এমন students দের খুঁজতে চাই যারা হয় `class: "10"` অথবা `result: "A"`।
...
**`$unset`** অপারেটর ব্যবহার করে কোনো ফিল্ড ডকুমেন্ট থেকে পুরোপুরি সরিয়ে ফেলা হয়। এটি ফিল্ডকে ডকুমেন্ট থেকে মুছে দেয়।
...
- ফিল্ডের নাম উল্লেখ করতে হয়, এবং এর মান হিসেবে ফাঁকা স্ট্রিং `""` বা যেকোনো কিছু দেওয়া যায়। 

#### **Example **:
ধরা যাক, `students` কালেকশনে প্রতিটি শিক্ষার্থীর তথ্য রয়েছে। আমরা একটি শিক্ষার্থীর `address` ফিল্ড মুছে দিতে চাই।
...
##### **Result**:
ডকুমেন্ট থেকে `address` ফিল্ডটি মুছে যাবে।

---

###  **`$pop` অপারেটর**

**`$pop`** অপারেটর ব্যবহার করে অ্যারে ফিল্ড থেকে একটি আইটেম সরানো হয়। এটি:
- অ্যারের **শেষের আইটেম** সরানোর জন্য `1` ব্যবহার করে।  
- অ্যারের **শুরুর আইটেম** সরানোর জন্য `-1` ব্যবহার করে।
...
- `<value>`: `1` (শেষ আইটেম সরাতে) বা `-1` (প্রথম আইটেম সরাতে)।

#### **Example **:
ধরা যাক, `orders` নামে একটি কালেকশনে প্রতিটি অর্ডারের `items` নামে একটি অ্যারে ফিল্ড রয়েছে। আমরা একটি অর্ডারের `items` অ্যারে থেকে শেষ আইটেম সরাতে চাই।
...
##### **Result**:
`items` অ্যারে থেকে শেষ আইটেমটি সরানো হবে।

#### **Example 2** (শুরুর আইটেম সরানো):
```javascript
db.orders.updateOne(
  { "_id": 101 },
  { "$pop": { "items": -1 } }
...
##### **Result**:
`items` অ্যারে থেকে প্রথম আইটেমটি সরানো হবে।

---

###  **`$pull` অপারেটর** (সম্ভাব্যভাবে **`popAll`** নামে বোঝানো হয়েছে)

MongoDB-তে **`$pull`** ব্যবহার করে অ্যারে থেকে এক বা একাধিক নির্দিষ্ট মান বা শর্ত অনুসারে মান সরানো হয়। এটি অ্যারের নির্দিষ্ট মানগুলো মুছে ফেলে।
...
- `<condition>`: একটি নির্দিষ্ট মান, অথবা একটি শর্ত।

#### **Example **:
ধরা যাক, `products` নামে একটি কালেকশন আছে যেখানে প্রতিটি পণ্যের `tags` নামে একটি অ্যারে রয়েছে। আমরা `tags` অ্যারে থেকে `sale` মুছে ফেলতে চাই।
...
##### **Result**:
`tags` অ্যারে থেকে `sale` মুছে যাবে।

#### **Example 2** (শর্ত অনুসারে মান মুছে ফেলা):
ধরা যাক, `orders` কালেকশনে `items` অ্যারে রয়েছে। আমরা `items` থেকে ১০-এর কম পরিমাণের পণ্য সরাতে চাই।
...
##### **Result**:
`items` অ্যারে থেকে সব পণ্য মুছে যাবে যার `quantity` ১০-এর কম।
...
| অপারেটর    | কাজ                                             | উদাহরণ                                                                                  |
|------------|-------------------------------------------------|-----------------------------------------------------------------------------------------|
| `$unset`   | ফিল্ড ডকুমেন্ট থেকে পুরোপুরি মুছে দেয়।          | `{ "$unset": { "fieldName": "" } }`                                                    |
| `$pop`     | অ্যারে থেকে প্রথম বা শেষ আইটেম সরিয়ে দেয়।       | `{ "$pop": { "arrayField": 1 } }` (শেষ আইটেম), `{ "$pop": { "arrayField": -1 } }` (প্রথম আইটেম) |
| `$pull`    | অ্যারে থেকে নির্দিষ্ট মান বা শর্ত অনুযায়ী মান সরিয়ে দেয়।| `{ "$pull": { "arrayField": <condition> } }`                                           |

---

### উদাহরণ: একাধিক অপারেটর একসঙ্গে ব্যবহার

ধরা যাক, `students` নামে একটি কালেকশন আছে। আমরা নিম্নলিখিত কাজ করতে চাই:
1. `address` ফিল্ড ডকুমেন্ট থেকে মুছে ফেলা।  
2. `subjects` অ্যারে থেকে শেষ সাবজেক্ট মুছে ফেলা।  
3. `hobbies` অ্যারে থেকে `sports` মুছে ফেলা।  
...
##### **Result**:
- `address` ফিল্ড মুছে যাবে।  
- `subjects` অ্যারে থেকে শেষ সাবজেক্ট সরানো হবে।  
- `hobbies` অ্যারে থেকে `sports` সরানো হবে। 
...
**`$` পজিশনাল অপারেটর** অ্যারে-র মধ্যে শর্ত মিলিয়ে প্রথম উপাদানের জন্য কাজ করে। এটি ব্যবহার করা হয় অ্যারে-তে নির্দিষ্ট শর্ত পূরণকারী একটি উপাদানের জন্য মান আপডেট করতে।
...
- **`$`**: অ্যারের সেই উপাদানের পজিশনকে নির্দেশ করে যেটি কোয়েরির শর্ত অনুযায়ী মিলে।

#### **Example (উদাহরণ)**:
ধরা যাক, আমাদের `students` নামে একটি কালেকশন আছে। প্রতিটি শিক্ষার্থীর `grades` নামে একটি অ্যারে রয়েছে, যেখানে প্রতিটি গ্রেডে `subject` এবং `marks` ফিল্ড রয়েছে। আমরা `Math` সাবজেক্টের নম্বর আপডেট করতে চাই।
...
##### **Result**:
এই কোয়েরি `grades` অ্যারে-তে প্রথম এমন আইটেম খুঁজে পাবে যেখানে `subject` **`Math`**, এবং তার `marks` ফিল্ড ৯০-এ আপডেট হবে।

---

###  **`$inc` অপারেটর**

**`$inc`** অপারেটর ব্যবহার করে কোনো সংখ্যাসূচক ফিল্ডের মান নির্দিষ্ট সংখ্যায় বৃদ্ধি বা হ্রাস করা যায়।  
...
- **`field`**: যেই ফিল্ডের মান আপডেট করতে হবে।  
- **`<amount>`**: যে পরিমাণে বৃদ্ধি (+ve) বা হ্রাস (-ve) করতে হবে।

#### **Example (উদাহরণ)**:
ধরা যাক, আমাদের `employees` নামে একটি কালেকশন আছে। আমরা একজন কর্মীর `salary` ৫০০০ টাকা বাড়াতে চাই।
...
##### **Result**:
এই কোয়েরি কর্মীর বর্তমান `salary` ফিল্ডের মান ৫০০০ টাকা বৃদ্ধি করবে।

#### **Example 2** (মান কমানো):
ধরা যাক, আমরা একটি পণ্যের `stock` ১০টি কমাতে চাই।
...
##### **Result**:
`stock` ফিল্ডের মান ১০টি কমে যাবে।

---

### **`$` এবং `$inc` একসঙ্গে ব্যবহারের উদাহরণ**

ধরা যাক, আমাদের `students` কালেকশনে প্রতিটি শিক্ষার্থীর `grades` অ্যারে রয়েছে। আমরা `Math` সাবজেক্টের নম্বর ৫ বাড়াতে চাই।
...
##### **Result**:
এই কোয়েরি `grades` অ্যারে-র সেই উপাদানের **`marks`** ফিল্ড ৫ বৃদ্ধি করবে যেখানে `subject` **`Math`**।
...
| অপারেটর    | কাজ                                             | উদাহরণ                                                                                 |
|------------|-------------------------------------------------|----------------------------------------------------------------------------------------|
| `$`        | অ্যারে-তে শর্ত মেলে এমন প্রথম উপাদানের পজিশনে কাজ করে। | `{ "$set": { "grades.$.marks": 90 } }`                                                |
| `$inc`     | সংখ্যাসূচক মান বৃদ্ধি বা হ্রাস করে।              | `{ "$inc": { "salary": 5000 } }` (বৃদ্ধি), `{ "$inc": { "stock": -10 } }` (হ্রাস)     |

---

### বাস্তব উদাহরণ: একাধিক অপারেটর ব্যবহার

ধরা যাক, `products` নামে একটি কালেকশনে প্রতিটি পণ্যের `reviews` নামে একটি অ্যারে রয়েছে। আমরা করতে চাই:
1. `reviews` অ্যারে-র যেখানে `user: "Rahim"` আছে, সেই উপাদানের `rating` ১ বাড়ানো।  
2. সেই পণ্যের `stock` ৫০ কমানো।  
...
##### **Result**:
- `reviews` অ্যারে-তে `user: "Rahim"` এর রেটিং ১ বাড়ানো হবে।  
- সেই পণ্যের `stock` ফিল্ড থেকে ৫০ কমানো হবে। 
